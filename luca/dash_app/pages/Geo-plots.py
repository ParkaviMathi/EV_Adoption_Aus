...
File_Path = "cleaned_data/Population_Income_Registration(8).csv"
...
layout = html.Div([
   ...
   # dcc = Dash Core Components -> Gives you access to many interactive components including dropdowns, checklists etc
    ...
    dbc.Col(
        dcc.Checklist(id="toggle_size_1",options=['Toggle Size'], value=[]) ) ] 
        ),

    html.Div(id='output_container_1', children=[]),

    dcc.Graph(id='graph_1', figure={}, style={'width': '70vw', 
                                                 'height': '70vh',
                                                 'margin': '0 auto'}),
    
    html.Br(),
    dcc.Graph(figure=fig_2, style={'width': '70vw', 
                                    'height': '70vh',
                                    'margin': '0 auto'}),

    html.Br(),
    dcc.Graph(figure=fig_8, style={'width': '70vw', 
                                    'height': '70vh',
                                    'margin': '0 auto'}),
    
    html.Br(),
    dcc.Graph(figure=fig_10, style={'width': '70vw', 
                                    'height': '70vh',
                                    'margin': '0 auto'})
...
def update_graph(option_slctd_1, toggle_size_1):
    ...
    container_1 = "The type chosen by user was: {}".format(option_slctd_1)

    if option_slctd_1 != 'All':
        dff_1 = dff_1[dff_1["Fuel Type"] == option_slctd_1]

    if len(toggle_size_1) == 0:
        size_opt_1 = None
    else:
